...
## Current Priority — Cruceta-First High-Throughput Roadmap

**Product outcome:** Within four months, build the corrected domain foundation (Titulares, Pasos, Route Sections, per-Paso Crucetas) and route-aware Cruceta, then prove that the same system can sustain 1,000 HTTP req/s and 500 events/s at p95 <300 ms with event freshness <5 s on disposable multi-node infrastructure. Product first; then provable foundations.

> **AS-IS vs TARGET:** Currently no Titular, Paso, Route Section, or per-Paso Cruceta exists. The plan builds toward the TARGET domain model. See `docs/agents/domain.md` for the canonical glossary and `docs/functional-map.md` §0.10 for the status table.

**Detailed plan:** [`2026-07-28-cruceta-first-high-throughput-roadmap.md`](plans/2026-07-28-cruceta-first-high-throughput-roadmap.md)

**Phase structure:**
- **Month 1 — Correct Domain Foundation and Route-Aware Cruceta:** Titulares (Hermandad-owned, projected into Procesion), ordered Pasos (each referencing one Titular), shared Route Sections (named occurrences; names may repeat), plan finalization (idempotent, makes Pasos/Route immutable), finalized plan projection into Repertorio, per-Paso Cruceta (Marchas assigned by Route Section with sequenceWithinSection), independent per-Paso run-sheet progression, Gate 07.
- **Month 2 — Correctness & Event Foundations:** Reliability contracts, Procesion tenant isolation (covering Paso, Route Section, finalization endpoints), versioned event envelope and keyed sender, optimistic lock repair, Hermandad/Procesion→Repertorio/Repertorio event migration (including Titular events, ProcesionPlanFinalizedEvent, per-Paso Cruceta events), correctness gate (Gate 15).
- **Month 3 — Reliable Delivery & Operability:** Multi-replica-safe outbox (SELECT FOR UPDATE SKIP LOCKED), atomic consumer dedup, Prometheus metrics, VPS deployment, deploy automation, reliable-deployment gate (Gate 21).
- **Month 4 — Load/Failure Evidence:** k6 load harness (including multiple Pasos with independent Crucetas), multi-node test environment, baseline capture, measured tuning, throughput proof (1,000 req/s, 500 events/s, expanded Cruceta workflow), Gate 26.

**22 implementation tickets** plus 4 integration gates — see the plan for full details.

**Explicitly deferred:** GPS triggers, maps, public tracking, push notifications remain deferred indefinitely. Rain/emergency route amendment after finalization is also deferred (see future backlog below).
...
### Future: Emergency/Rain Route Amendment (deferred)

Versioned route amendment after plan finalization. Scope to be determined when active roadmap reaches this item.

**Minimum known concerns:**
- Versioned route amendment (new route replaces finalized route, old route preserved for reference).
- Explicit review/remapping or invalidation of affected per-Paso Crucetas — no silent remapping.
- If a Paso's relevant Route Sections change, affected Cruceta Items must be flagged for operator review.

**Explicitly not designed now:** Do not design rain amendment logic or schema until this backlog item is picked up. The MVP assumes immutable finalized plans.
